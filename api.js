...
process.on(
  "uncaughtException",
  (err) => process.argv.includes("--deploy") && alertServerError(err)
);
...
const {
  ...
  logNewRequest,
  checkRoute,
} = require("./assets/api_funcs");
const { points } = require("./assets/points");
console.log("--");
const { CouchDB } = require("./assets/couchdb");
const { startIpListener } = require("./assets/ip_address");

startIpListener(); // Keep the server ip up to date
...
app.listen(port, () => {
  ...
  logger.log({
    msg: "Started api and listening to port " + port,
    timestamp: Date.now(),
  });
});
